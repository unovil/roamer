  ...
  >
    ...
    {#if !(typeof form?.error == "undefined" && form?.response.schoolName && form?.response.sections.length != 0)}
      ...
      >
        ...
        <button
          type="button"
          class="items-center justify-center w-40 h-10 border border-gray-400 rounded-md {role ==
          'ADMIN'
            ? 'bg-green-700'
        ...
        <button
          type="button"
          class="items-center justify-center w-40 h-10 ml-6 border border-gray-400 rounded-md {role ==
          'STUDENT'
            ? 'bg-green-700'
            : 'hover:bg-green-500'}"
          on:click={() => {
