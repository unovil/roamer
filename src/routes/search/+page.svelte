...
<div class="flex justify-center items-start mt-20 mb-10 font-bold text-6xl">
  <h1> Roaming for something? </h1>
</div>

<form class="flex flex-row items-center justify-center space-x-2 rounded">
  <!-- svelte-ignore a11y-autofocus -->
  <input type="text" bind:value={searchTerm} autofocus placeholder="Search..." class ="w-2/5 rounded-md" />
  <select name="searchCategory" bind:value={searchCategory} class = "rounded-md bg-green-100">
...
{#if searchTerm}
  <div class="border-2 border-gray-300 p-4 rounded-lg w-3/5 mx-auto mt-10 h-full justify-items-center">
    <table>
      {#if typeof results !== "undefined" && results !== null}
        {#each results as result (result.id)}
          <tr>
            <td><img src={result.image} alt={result.name} class="w-20 h-20" />
            <td>
