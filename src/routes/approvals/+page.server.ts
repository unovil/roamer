...
export const load: PageServerLoad = async (event) => {
    ...
    const requests = await db.request.findMany({
        ...
        select: {
            ...
            requestDates: true,
            description: true,
            facility: { select: { name: true, id: true, blockedDates: true, image: true } },
            equipment: { select: { name: true, id: true, blockedDates: true, image: true } },
            students: {
                include: {
...
export const actions = {
    approve: async ({ request }) => {
        ...
        if (type === "facility") {
            let blockedDates = Array.isArray(placeRequest.facility?.blockedDates) ? placeRequest.facility?.blockedDates : [];
